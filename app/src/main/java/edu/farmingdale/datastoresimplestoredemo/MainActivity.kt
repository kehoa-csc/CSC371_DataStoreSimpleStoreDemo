    ...
    Column (modifier = Modifier.padding(50.dp)) {
        ...
        Button(onClick = {
            coroutineScope.launch {
                store.saveUsername(username = username)
                store.saveHighscore(24)
                store.saveDarkModePref(false)
            }
...
@Composable
fun DataStoreDemoPreview() {
    DataStoreSimpleStoreDemoTheme() {
        DataStoreDemo(modifier = Modifier)
    }
}

// ToDone 1: Modify the App to store a high score and a dark mode preference
// ToDone 2: Modify the APP to store the username through a text field
// ToDone 3: Modify the App to save the username when the button is clicked
// ToDone 4: Modify the App to display the values stored in the DataStore
